...
#e)
SELECT desc_convenio, COUNT(cd_pac)
FROM convenio JOIN paciente ON cod_convenio = conv_pac
GROUP BY desc_convenio
ORDER BY desc_convenio;

#f)
SELECT nm_pac, dt_nasc
FROM paciente p
WHERE(	SELECT COUNT(*)
		FROM atendimento a
        WHERE a.cd_pac = p.cd_pac) > 2;
