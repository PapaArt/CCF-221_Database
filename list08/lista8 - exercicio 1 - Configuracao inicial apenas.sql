...
FROM paciente NATURAL JOIN atendimento
...
SELECT *
FROM paciente 
WHERE cd_pac NOT IN (	SELECT cd_pac
						FROM paciente NATURAL JOIN atendimento);
...
FROM convenio
WHERE cod_convenio IN ( SELECT conv_pac
						FROM paciente);

#e)
SELECT *
FROM convenio
WHERE cod_convenio NOT IN (	SELECT conv_pac
							FROM paciente);
...
SELECT cd_pac, nm_pac, dt_nasc, desc_convenio
    ...
    UNION 
        ...
        WHERE cod_convenio NOT IN (	SELECT cod_convenio
									FROM paciente JOIN convenio ON conv_pac = cod_convenio)
