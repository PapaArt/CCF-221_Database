...
SELECT DISTINCT desc_convenio, COUNT(*) AS qtd_pacientes
FROM convenio JOIN paciente ON conv_pac = cod_convenio
GROUP BY desc_convenio;

#b)
SELECT *
FROM conveniopacientes
WHERE qtd_pacientes > 2;

#c)
CREATE VIEW pacientesatendidos AS
SELECT DISTINCT nm_pac, dt_nasc
FROM paciente
WHERE (	SELECT COUNT(*)
		FROM atendimento NATURAL JOIN paciente) > 2;
        
#d)
SELECT *
FROM pacientesatendidos
WHERE nm_pac LIKE 'S%' OR nm_pac LIKE 'R%';